...
def create_ego_raster(vehicle_state):
    ...
    # Paint the rectangle
    rect = plt.Rectangle(ego_bottom_right, ego_front_length+ego_rear_length, ego_width, linewidth=2, color='c', alpha=0.6, zorder=3,
...
def create_agents_raster(agents):
    for i in range(agents.shape[0]):
        if agents[i, 0] != 0:
            ...
            rect = plt.Rectangle(agent_bottom_right, agent_length, agent_width, linewidth=2, color='#FFE000', alpha=0.6, zorder=3,
                                transform=mpl.transforms.Affine2D().rotate_around(*(x_center, y_center), heading) + plt.gca().transData)
            plt.gca().add_patch(rect)
...
def create_map_raster(lanes, crosswalks, route_lanes):
    for i in range(lanes.shape[0]):
        lane = lanes[i]
        if lane[0][0] != 0:
            plt.plot(lane[:, 0], lane[:, 1],  'w--', linewidth=2) # plot centerline
    ...
    for k in range(route_lanes.shape[0]):
        ...
        if route_lane[0][0] != 0:
            plt.plot(route_lane[:, 0], route_lane[:, 1], '#61BE44', linewidth=4) # plot route_lanes green


def draw_trajectory(ego_trajectory, agent_trajectories):
    # plot ego 
    plt.plot(ego_trajectory[:, 0], ego_trajectory[:, 1], 'c', linewidth=3, zorder=3)

    # plot others
    for i in range(agent_trajectories.shape[0]):
        if agent_trajectories[i, -1, 0] != 0:
            trajectory = agent_trajectories[i]
            plt.plot(trajectory[:, 0], trajectory[:, 1], '#FFE000', linewidth=3, zorder=3)
            
def draw_ego_trajectory(ego_trajectory):
    # plot ego 
