...
class DataProcessor(object):
    def __init__(self, scenarios):
        ...
        self.num_past_poses = 5 * self.past_time_horizon 
        self.future_time_horizon = 8 # [seconds]
        self.num_future_poses = 5 * self.future_time_horizon
        self.num_agents = 20
...
def plot_single_scenario(data, ego_agent_future):
    # Create map layers
    ...
    draw_trajectory(data['ego_agent_past'], data['neighbor_agents_past'])
    draw_trajectory(data['ego_agent_future'], data['neighbor_agents_future'])
    draw_ego_trajectory(ego_agent_future[0])

    plt.gca().set_facecolor('#222429')
    # plt.gca().margines(0)
    plt.gca().set_aspect('equal')
    plt.tight_layout()
    plt.show()   
    
def plot_single_scenario_backup(data):
    # Create map layers
    create_map_raster(data['map_lanes'], data['map_crosswalks'], data['route_lanes'])
